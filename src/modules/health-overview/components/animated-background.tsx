...
import Animated, { useAnimatedStyle, interpolate, useAnimatedProps } from "react-native-reanimated";
...
export function AnimatedBackground({ pulseProgress, colors, blobSize = 280 }: AnimatedBackgroundProps) {
  const defaultColors = {
  ...
  return (
    <View className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      {/* Top gradient - wider for hazardous state */}
      <LinearGradient
        colors={finalColors.topGradient}
        style={{
          position: "absolute",
          top: blobSize > 300 ? -80 : 0,
          left: 0,
          right: 0,
          height: blobSize > 300 ? "50%" : "40%",
          opacity: blobSize > 300 ? 0.6 : 1,
        }}
        pointerEvents="none"
      />
      ...
      {blobSize > 300 ? (
        // Organic SVG blob for hazardous state
        <Animated.View
          style={[
            {
              position: "absolute",
        ...
        // Simple circular gradient for safe/moderate states
        <View
          style={{
            position: "absolute",
            top: "50%",
